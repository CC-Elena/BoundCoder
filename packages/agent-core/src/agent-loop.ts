import type { AgentRunOptions, AgentRunResult, AgentMessage } from "@boundcoder/shared";
...
const DEFAULT_MAX_STEPS = 5;export function runAgentLoop(options: AgentRunOptions): AgentRunResult{
  ...
  for(let loopCount = 0; loopCount < maxSteps; loopCount++) {
    const modelResponse = fakeModel(messages);
    messages.push(modelResponse);
     ...
     if(modelResponse.kind === 'text') {
        ...
        const toolResult = executeFakeTool(modelResponse.toolCall);
        messages.push({
          role: 'tool',
          kind: 'tool_result',
